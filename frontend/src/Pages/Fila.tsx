...
function FilaGeral() {
    ...
    const fetchStatus = async () => {
        ...
        return {
            ...
            fila: res.data.fila.map((user: any) => {
                return createUser(user.name, user.age, user.sex, user.telegram_id, user.username, user.phone, user.timestamp);
            })
        };
    };
    ...
    function createUser(nome: any, idade: any, sexo: any, telegram_id: any, username: any, phone: any, timestamp: any): User {
        ...
        if (timestamp) {
            ...
            waiting_time = Math.floor((now - t) / (1000 * 60));
        }
        return { nome, idade, sexo, telegram_id, username, phone: identedphone, waiting_time };
    }
                                        ...
                                        {filaAtendimento.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row, idx) => {
                                            return (
                                                <TableRow hover role="checkbox" tabIndex={-1} key={row.telegram_id}>
                                                    ...
                                                    <TableCell
                                                        align='center'>
                                                        {filaAtendimento[idx].waiting_time}m
                                                    </TableCell>
                                                </TableRow>
                                            );
                                        })}
