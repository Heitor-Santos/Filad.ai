import React, {useState} from 'react';
import Container from '@material-ui/core/Container'
import './Fila.css'
import {
    makeStyles, Paper,
    Table,
    TableBody,
    TableCell,
    TableContainer,
    TableHead,
    TablePagination,
    TableRow,
    Dialog,
    DialogActions,
    DialogContent,
    DialogContentText,
    DialogTitle,
    Button
} from "@material-ui/core";

interface User {
    nome: any;
    idade: any;
    sexo: any;
    telegram_id: any;
}

function FilaGeral() {
    const [filaSolicitacao, SetFilaSolicitacao] = useState([
        createUser('Andre Vsasconcelos', '21', 'M', 'andre69'),
        createUser('Andre Vasconcelos', '21', 'M', 'andre59'),
        createUser('Andre Vasconcelos', '21', 'M', 'andre49'),
        createUser('Andre Vasconcelos', '21', 'M', 'andre39'),
        createUser('Andre Vasconcrelos', '21', 'M', 'andr269'),
        createUser('Andre Vassconcelos', '21', 'M', 'andr169'),
        createUser('Andre Vasconcelos', '21', 'M', 'andre89'),
        createUser('Andre Vascowncelos', '21', 'M', 'andr069'),
        createUser('Andre Vsasconcelos', '21', 'M', 'and2e69'),
        createUser('Andre Vasconcelos', '21', 'M', 'andr1e69'),
        createUser('Andre Vsascofncelos', '21', 'M', 'an4dre69'),
        createUser('Andre Vasconcelos', '21', 'M', 'andr5e69'),
        createUser('Andre Vascodncelos', '21', 'M', 'an7dre69'),
        createUser('Andre Vasconcelos', '21', 'M', 'and8re69'),
        createUser('Andre Vasconcelos', '21', 'M', 'andr9e69'),
    ]);
    const [filaAtendimento, SetFilaAtendimento] = useState([
        createUser('Andre Vsasconcelos', '21', 'M', 'andre69'),
        createUser('Andre Vasconcelos', '21', 'M', 'andre59'),
        createUser('Andre Vasconcelos', '21', 'M', 'andre49'),
        createUser('Andre Vasconcelos', '21', 'M', 'andre39'),
        createUser('Andre Vasconcrelos', '21', 'M', 'andr269'),
        createUser('Andre Vassconcelos', '21', 'M', 'andr169'),
    ]);
    const [open, setOpen] = React.useState(false);

    const handleClickOpen = () => {
        setOpen(true);
    };

    const handleClose = () => {
        setOpen(false);
    };

    interface Column {
        id: 'nome' | 'idade' | 'sexo';
        label: string;
        minWidth?: number;
        align?: 'right' | 'center';
        format?: (value: number) => string;
    }

    const columns: Column[] = [
        { id: 'nome', label: 'Nome', minWidth: 130 },
        { id: 'idade', label: 'Idade', minWidth: 60, align: 'center' },
        { id: 'sexo', label: 'Sexo', minWidth: 60, align: 'center' }
    ];

    function createUser(nome: any, idade: any, sexo: any, telegram_id: any): User {
        return { nome, idade, sexo, telegram_id };
    }

    const useStyles = makeStyles({
        root: {
            width: '100%',
        },
        container: {
            maxHeight: 500,
        },
    });

    const classes = useStyles();
    const [page, setPage] = useState(0);
    const [rowsPerPage, setRowsPerPage] = useState(5);

    const handleChangePage = (event: unknown, newPage: number) => {
        setPage(newPage);
    };

    const handleChangeRowsPerPage = (event: React.ChangeEvent<HTMLInputElement>) => {
        setRowsPerPage(+event.target.value);
        setPage(0);
    };

    const labelRows = (page: number, rowsPerPage:number, length:number) => {
        return `${page * rowsPerPage + 1}-${page * rowsPerPage + rowsPerPage} de ${length}`
    }

    //não implementado ainda
    function acceptUser(user: any) {
        SetFilaAtendimento([...filaAtendimento,user])
        let aux = filaSolicitacao
        aux.splice(filaSolicitacao.indexOf(user),1)
        SetFilaSolicitacao([...aux])
    }
    //não implementado ainda
    function rejectUser(user: any) {
        let aux = filaSolicitacao
        aux.splice(filaSolicitacao.indexOf(user),1)
        SetFilaSolicitacao([...aux])
        handleClose()
    }

    return (
        <Container>
            <div className="box-superior">
                <div className="box-esquerda">
                    <p>
                        <h1>Fila</h1>
                    </p>
                    <p id="white">
                        Visão geral da fila
                    </p>
                </div>
                <div className="box-direita">
                    <div className="clock">
                        <img className="clock-image" src="data:image/png;base64,"/>
                        <p className="clock-text">
                            Tempo médio de espera
                        </p>
                        {/*pegar info do back*/}
                        <p>
                            Tempo médio
                        </p>
                    </div>
                    <div className="pacientes">
                        <p style={{marginBottom: '1rem'}}>
                            Pacientes hoje
                        </p>
                        {/*pegar info do back*/}
                        <p>
                            Num. pacientes
                        </p>
                    </div>
                </div>
            </div>
            <div className="box-inferior">
                {/*fila solicitação*/}
                <div style={{margin: '0 10px'}}>
                    <div>
                        <div style={{display: 'flex', justifyContent: 'space-between'}}>
                            <h3>Novos pedidos de atendimento</h3>
                            <h4>{filaSolicitacao.length}</h4>
                        </div>
                        {/*atualizar a tabela após aceitação/remoção de usuario*/}
                        <Paper className={classes.root}>
                            <TableContainer className={classes.container}>
                                <Table stickyHeader aria-label="sticky table">
                                    <TableHead>
                                        <TableRow>
                                            {columns.map((column) => (
                                                <TableCell
                                                    key={column.id}
                                                    align={column.align}
                                                    style={{ minWidth: column.minWidth }}
                                                >
                                                    {column.label}
                                                </TableCell>
                                            ))}
                                            <TableCell />
                                        </TableRow>
                                    </TableHead>
                                    <TableBody>
                                        {filaSolicitacao.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                                            return (
                                                <TableRow hover role="checkbox" tabIndex={-1} key={row.telegram_id}>
                                                    {columns.map((column) => {
                                                        const value = row[column.id];
                                                        return (
                                                            <TableCell key={column.id} align={column.align}>
                                                                {column.format && typeof value === 'number' ? column.format(value) : value}
                                                            </TableCell>
                                                        );
                                                    })}
                                                    <TableCell>
                                                        <img style={{width: '20px', cursor: 'pointer'}} onClick={() => handleClickOpen()} src="data:image/png;base64,"/>
                                                        <Dialog
                                                            open={open}
                                                            onClose={handleClose}
                                                            aria-labelledby="alert-dialog-title"
                                                            aria-describedby="alert-dialog-description"
                                                        >
                                                            <DialogContent>
                                                                <DialogContentText id="alert-dialog-description">
                                                                    Deseja realmente recusar este paciente?
                                                                </DialogContentText>
                                                            </DialogContent>
                                                            <DialogActions>
                                                                <Button onClick={() => rejectUser(row)} color="primary">
                                                                    Sim
                                                                </Button>
                                                                <Button onClick={handleClose} color="primary" autoFocus>
                                                                    Não
                                                                </Button>
                                                            </DialogActions>
                                                        </Dialog>
                                                        <img style={{width: '18px', cursor: 'pointer'}} onClick={() => acceptUser(row)} src="data:image/png;base64,"/>
                                                    </TableCell>
                                                </TableRow>
                                            );
                                        })}
                                    </TableBody>
                                </Table>
                            </TableContainer>
                            <TablePagination
                                labelRowsPerPage='Linhas por página:'
                                labelDisplayedRows={() => labelRows(page, rowsPerPage, filaSolicitacao.length)}
                                rowsPerPageOptions={[10,25,100]}
                                component="div"
                                count={filaSolicitacao.length}
                                rowsPerPage={rowsPerPage}
                                page={page}
                                onPageChange={handleChangePage}
                                onRowsPerPageChange={handleChangeRowsPerPage}
                            />
                        </Paper>
                    </div>
                </div>
                {/*fila em andamento*/}
                <div style={{margin: '0 10px'}}>
                    <div>
                        <div style={{display: 'flex', justifyContent: 'space-between'}}>
                            <h3>Fila em andamento</h3>
                            <h4>{filaAtendimento.length}</h4>
                        </div>
                        {/*atualizar a tabela após aceitação/remoção de usuario*/}
                        <Paper className={classes.root}>
                            <TableContainer className={classes.container}>
                                <Table stickyHeader aria-label="sticky table">
                                    <TableHead>
                                        <TableRow>
                                            {columns.map((column) => (
                                                <TableCell
                                                    key={column.id}
                                                    align={column.align}
                                                    style={{ minWidth: column.minWidth }}
                                                >
                                                    {column.label}
                                                </TableCell>
                                            ))}
                                            <TableCell />
                                        </TableRow>
                                    </TableHead>
                                    <TableBody>
                                        {filaAtendimento.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                                            return (
                                                <TableRow hover role="checkbox" tabIndex={-1} key={row.telegram_id}>
                                                    {columns.map((column) => {
                                                        const value = row[column.id];
                                                        return (
                                                            <TableCell key={column.id} align={column.align}>
                                                                {column.format && typeof value === 'number' ? column.format(value) : value}
                                                            </TableCell>
                                                        );
                                                    })}
                                                    <TableCell>
                                                        <img style={{width: '18px', cursor: 'pointer'}} src="data:image/png;base64," />
                                                        <img style={{width: '20px', cursor: 'pointer'}} src="data:image/png;base64,"/>
                                                        <img style={{width: '18px', cursor: 'pointer'}} src="data:image/png;base64,"/>
                                                    </TableCell>
                                                </TableRow>
                                            );
                                        })}
                                    </TableBody>
                                </Table>
                            </TableContainer>
                            <TablePagination
                                labelRowsPerPage='Linhas por página:'
                                labelDisplayedRows={() => labelRows(page, rowsPerPage, filaAtendimento.length)}
                                rowsPerPageOptions={[10,25,100]}
                                component="div"
                                count={filaAtendimento.length}
                                rowsPerPage={rowsPerPage}
                                page={page}
                                onPageChange={handleChangePage}
                                onRowsPerPageChange={handleChangeRowsPerPage}
                            />
                        </Paper>
                    </div>
                </div>
            </div>
        </Container>
    );
}
export default FilaGeral;
